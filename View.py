...
class View:
    def __init__(self, top=None):
        ...
        self.songName.configure(text='''Song Name Here''')
    ...
    def setup_player(self):
        ...
        self.Button10.configure(command=self.remove_songs_from_favourites)
        self.playList.configure(font="vivaldi 12")
        self.playList.bind("<Double-1>",self.list_double_click)
        img=tk.PhotoImage(file="F:\my Python code\Mouzikka\icons\like.png")
    ...
    def add_song(self):
        self.song_name = self.my_player.add_song()
        if self.song_name is None:
            return
        self.playList.insert(tk.END, self.song_name)
    ...
    def load_previous_song(self):
        try:
            if hasattr(self,"sel_song_index_tuple")==False:
                raise NoSongsSelectedError("please Select a Song First")
    ...
    def load_songs_from_favourites(self):
        try:
            ...
            print(mycolor)
            self.playList.configure(fg=mycolor)
            messagebox.showinfo("success!!","List populated from Favourites")
        except(DatabaseError)as ex1:
           messagebox.showerror("DB Error!","Sorry ")
    def remove_songs_from_favourites(self):
        song_fav=self.playList.curselection()
        try:
            if len(song_fav)==0:
                raise NoSongsSelectedError("Please select a sing first to remove")
            song_fav=self.playList.get(song_fav[0])
            result=self.my_player.remove_song_from_favourites(song_fav)
            if "deleted" in result:
                self.playList.delete(song_fav[0])
                messagebox.showinfo("Removed",result)
        except DatabaseError as ex2:
            messagebox.showerror("Error!","song can not be removed")
            print(ex2)
